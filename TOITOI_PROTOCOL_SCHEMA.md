...
**Data Structure as a Boundary Object (Version 1.2 - DSL Extension)**
...
**Version: 1.2** | Main updates from the previous version (v1.1):
...
From version 1.2, an inquiry in Toitoi is understood as having a **two-layer structure**:
...
The vocabulary defined in this document is for the initial version (v1.0/v1.2). As the system operates, unknown relationships and new contexts will be required.
...
**バージョン: 1.2**　｜　前バージョン (v1.1) からの主なアップデート：
...
バージョン1.2より、Toitoiにおける「問い」は**二層構造**として理解されます。
...
本システムは引き続き、既存の4層タグ構造（文脈・関係性・熟達フェーズ・系譜）でアグロエコロジーの知を記述します。これに加えて、バージョン1.2からは機械推論と将来的なナレッジグラフ統合を目的とした第5の構造層として「DSL」が加わります。
...
本ドキュメントで定義した語彙は初期バージョン（v1.0〜v1.2）のものです。システムが稼働するにつれ、未知の関係性や新たなコンテキスト、DSLの表現力拡張が必要になります。
