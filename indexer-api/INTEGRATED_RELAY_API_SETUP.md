...
**バージョン：1.1**　｜　前バージョン (v1.0) からの主な修正：Step 3.1: `ports:` の追加手順を具体化、`networks:` typo注意を追記。Step 3.2: コンテナ名の確認手順を追加（`nostream-db-1` → `nostream-db`）。Step 4.1: Prismaをv5系に固定、理由を明記。Step 4.3: `.env` の配置場所を明示、ディレクトリ構成図を追加。Section 7: バックアップコマンドのコンテナ名を修正。
